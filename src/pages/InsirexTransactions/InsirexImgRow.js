...
export default function InsirexImgRow({ el, handleOpen, setFullImg, transaction , rowId , setImgs,setRowId }) {
   ...
   async function DeleteImg(img_path) {
        try {
            ...
            const { data } = await $api.post('/deleteImgsInsirexTransation', { img_path })
    ...
    const EditImg = async (event, img_path) => {
        ...
        try {
            await $api.patch(`/editInsirexCheck/:${id}?login=${secureLocalStorage.getItem('userLogin')}&img_path=${img_path}&payment_method=Insirex`, img, {
                headers: {
