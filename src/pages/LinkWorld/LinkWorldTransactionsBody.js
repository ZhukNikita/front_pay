import styles from './LinkWorldTransactionsBody.module.scss'
import LinkWorldTransactionsList from './LinkWorldTransactionsList'
...
export default function LinkWorldTransactionsBody() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true)
      try {
        const { data } = await $api.post('/getLinkWorldTransactions',{brand: secureLocalStorage.getItem('userBrand')})
        if (data) {
  ...
  return (
    <div className={styles.body}>
      <div className={styles.header}>
        <h1>Транзакции LinkWorld</h1>
      </div>
      <LinkWorldTransactionsList transactions={transactions} isLoading={isLoading} setTransactions={setTransactions} />
