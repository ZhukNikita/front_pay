import styles from './LinkWorldTransactionsList.module.scss'
...
export default function LinkWorldTransactionsList({ transactions, isLoading, setTransactions }) {
  ...
  useEffect(() => {
    if (dateSort) {
      const sortedTransactions = [...transactions].sort((a, b) => new Date(a.order_id) - new Date(b.order_id));
      setTransactions(sortedTransactions);
      setStatusSort(null)
      setAmountSort(null)
    }
    if (dateSort === false) {
      const sortedTransactions = [...transactions].sort((a, b) => new Date(b.order_id) - new Date(a.order_id));
      setTransactions(sortedTransactions);
      setStatusSort(null)
      setAmountSort(null)
    }
  ...
  useEffect(() => {
    if (transactions.length > 0) {
      const filtered = transactions.filter(
        (transaction) => transaction.description?.toLowerCase().includes(search.toLowerCase())
      );
  ...
  return (
    <div className={styles.transactionsList}>
      ...
      <div className={styles.header}>
        ...
        <h3 style={{ width: '13.5vw' }}>Карта</h3>
