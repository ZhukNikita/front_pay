...
export default function RuCassaTransactionsList({ibans ,setIbans}) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                let {data} = await $api.post('/getAllListRuCassa' , {role: secureLocalStorage.getItem('role') , brand:secureLocalStorage.getItem('userBrand')})
                if(data){
        ...
        <div className={styles.transactionsList}>
            <div className={styles.search}>
                ...
                /> */}
                {
                    ...
                    :
                    <button ><Link to='/rucassa'><AddIcon />Создать транзакцию</Link></button>
                }
                {
                    isLoading && (<div style={{display:'flex' , alignItems:'center' , justifyContent:'center' , width:'100%'}}><LinearProgress sx={{ width: '75%' }} color='inherit' /></div>)
                }
            </div>
            <div className={styles.header}>
                <h3 className={styles.login}>Id
                </h3>
